...
@Service
class KospiOrderBookBroadcaster(
    private val kospiKafkaOrderBookReceiver: KafkaReceiver<String, KospiOrderBookDto>
) {
    // 멀티캐스트(multicast)가 가능한 Sink를 생성
    private val sink: Sinks.Many<KospiOrderBookDto> = Sinks.many().multicast().onBackpressureBuffer()

    // 애플리케이션 시작 시 카프카 메시지 수신을 시작
    @PostConstruct
    fun startKafkaConsumption() {
        kospiKafkaOrderBookReceiver.receive()
            .doOnNext { record ->
     ...
     */
    fun getStockDataStream(): Flux<KospiOrderBookDto> {
        return sink.asFlux()
    }
}